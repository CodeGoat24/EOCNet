...
def load_data(file_path, tokenizer):
    ...
    if data_dict['caption'] == "":
        prompt = prompt[:-1]
        data_dict['caption'] = prompt
    else:
    ...
    return label, class_ids, prompt, img_name, data_dict['caption']
...
def main():
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default="ckpt/eoc-sd-v1-4-coco.ckpt",
    ...
    parser.add_argument(
        "--json",
        type=str,
        default="examples/layout_82807.json",
    ...
    label, class_ids, prompt, img_name, caption = load_data(opt.json, tokenizer)
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                caption = model.get_learned_conditioning(caption)
                ...
                Image.fromarray(x_sample.astype(np.uint8)).save(os.path.join(outpath, f"{img_name}-{prompt}-{opt.seed}.jpg"))
