...
class GCCG(torch.nn.Module):
    ...
    def forward(self, feature):
        ...
        # return output
        ss = F.relu(self.conv(ss))
        ss = F.relu(self.conv1(ss))
        return ss
...
class CrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.):
        ...
        self.fc = nn.Linear(context_dim, inner_dim, bias=False)
        ...
        self.get_local_similarity = GCCG(context_size=5, output_channel = 77)
        self.register_parameter('alpha_local', nn.Parameter(torch.tensor(0.)))
    ...
    def forward(self, x, context=None, label=None, class_ids=None, mask=None):
        ...
        if exists(label) and exists(class_ids):
            ...
            for ii in range(B):
                ...
                for jj in range(len(ids)):
                    ...
                    if ids[jj] in [9, 11, 12, 13, 29, 36, 38, 39, 41, 42, 45, 57, 63, 66, 76, 87, 88, 90, 102, 113, 115, 116, 117, 144, 145, 170, 171, 173, 174, 175, 176, 178, 179]:
                        index.append(ids[jj][0])
                    if ids[jj] in [10, 157]:
                        index.append(ids[jj][0])
                        index.append(ids[jj][0])
            ...
            sim_local = self.get_local_similarity(q)
            sim_local = rearrange(sim_local, 'b c h w-> b (h w) c')
            sim_local = sim_local.softmax(dim = -1)
            out_local = einsum('b i j, b j d -> b i d', sim_local, v)
        ...
        if out_local != None:
            out = out + torch.tanh(self.alpha_local) * out_local
...
class LayoutFusionCrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.):
        super().__init__()
        inner_dim = dim_head * heads
        context_dim = default(context_dim, query_dim)
...
class LayoutFusionBlock(nn.Module):
    def __init__(self, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=True):
        super().__init__()
        self.attn1 = LayoutFusionCrossAttention(query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout)  # is a self-attention
        self.ff = FeedForward(dim, dropout=dropout, glu=gated_ff)
        self.attn2 = LayoutFusionCrossAttention(query_dim=dim, context_dim=context_dim,
                                    heads=n_heads, dim_head=d_head, dropout=dropout)  # is self-attn if context is none
        self.norm1 = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(dim)
        self.norm3 = nn.LayerNorm(dim)
...
class LayoutFusionModule(nn.Module):
    """
    Transformer block for image-like data.
    First, project the input (aka embedding)
    and reshape to b, t, d.
    ...
    def __init__(self, in_channels, n_heads, d_head,
        ...
        self.transformer_blocks = nn.ModuleList(
            [LayoutFusionBlock(inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim)
                for d in range(depth)]
        )
